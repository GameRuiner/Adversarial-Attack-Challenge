...
from .base_fr_model import BaseFRModel
...
class InsightFaceFRModel(BaseFRModel):
    def __init__(self, model_: torch.nn.Module,
                 preprocess: Callable=None,
                 device = "cpu",
                 align_models_root: str = None):
        super().__init__(model_, preprocess, device)
        self.models_root = align_models_root  # for face alignment and normal
        self.preprocess = self.create_preprocess()

    def create_preprocess(self):
        """
        Create a face alignment + normalization preprocess function for the insightface model
...
def load_insightface_ir50_model(model_path: str,
                                align_models_root: str = None,
                                device: str = 'cuda:0'):
    model = IR_50([112,112])
